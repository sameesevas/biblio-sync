...
public class AddBook extends JFrame implements ActionListener{
	...
	contentPane.setLayout(null);
        ...
        JLabel l0 = new JLabel("Catagory");
	l0.setForeground(new Color(47, 79, 79));
	l0.setFont(new Font("Tahoma", Font.BOLD, 14));
	l0.setBounds(73, 20, 90, 22);
	contentPane.add(l0);
	...
	contentPane.add(l7);
        ...
        t0 = new JTextField();
	t0.setForeground(new Color(47, 79, 79));
	t0.setFont(new Font("Trebuchet MS", Font.BOLD, 14));
	t0.setBounds(169, 25, 198, 20);
	contentPane.add(t0);
	t0.setColumns(10);
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            if(ae.getSource() == b1){
                ...
                if(t0.getText().equalsIgnoreCase("CSE"))
                {
                    
                    String sql = "insert into cse(book_id, name, isbn, publisher, edition, price, pages) values(?, ?, ?, ?, ?, ?, ?)";
		...
		st.close();
                ...
                else if(t0.getText().equalsIgnoreCase("CE"))
                {
                    String sql = "insert into ce(book_id, name, isbn, publisher, edition, price, pages) values(?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
                // st.setInt(1, Integer.parseInt(textField.getText()));
		st.setString(1, t1.getText());
		st.setString(2, t2.getText());
		...
		st.close();
                 ...
                 else if(t0.getText().equalsIgnoreCase("EECE"))
                {
                    String sql = "insert into eece(book_id, name, isbn, publisher, edition, price, pages) values(?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
                // st.setInt(1, Integer.parseInt(textField.getText()));
		st.setString(1, t1.getText());
		st.setString(2, t2.getText());
		...
		st.close();
                 ...
                 else if(t0.getText().equalsIgnoreCase("ME"))
                {
                    String sql = "insert into me(book_id, name, isbn, publisher, edition, price, pages) values(?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
                // st.setInt(1, Integer.parseInt(textField.getText()));
		st.setString(1, t1.getText());
		st.setString(2, t2.getText());
